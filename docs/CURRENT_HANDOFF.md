...
- 每日 HTML 展示方案（`script/050 mailtxt.py`、`pipeline/usage_trends.py`）：不再输出颜色提示、统计范围、取样逻辑等解释文字；H3/M3 仅显示 `YYYY-MM-DD` 并采用弱化紧凑样式；汇总左组顺序为出库、入库、外仓库存总量、家里库存总量，右组顺序为月计划、月计划预估还有要发货、月计划缺口排产。月度契约中的同名说明已同步更新。
