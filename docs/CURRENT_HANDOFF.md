...
- 运行复核发现：`35421736354` 的每日步骤 033 读取了远端旧自动清单（工作表 `2608`），只把库存表头改成“3月2周均”，没有在日报流程中调用月度清单生成公式；因此该次邮件的外应存/家应存数值不能证明已切换到新口径。
- 新增 `.github/workflows/publish-auto-list.yml` 与 `tools/publish_auto_list.py`：只识别旧“1周”或“3月周均”表头，将两列数值各乘 2 后发布当前 WebDAV 自动清单；新口径或未知表头会停止，且不会下载邮箱、生成日报、发送邮件或写月度报告。
- 自动清单发布运行 [35423340274](https://github.com/huozao/meidi-auto/actions/runs/35423340274) 已于 2026-09-19 成功完成：远端清单实际表头为“外应存(1周)”/“家应存(1周)”，共调整 49 行，WebDAV 文件 SHA256 前缀从 `d7b64f103c0f` 变为 `672aa5d97c24`。该工作流只发布清单，未执行任何邮箱、日报或月度报告步骤。
- 此前两次只发布运行 `35422949765`（入口导入路径）和 `35423049642`（安全识别旧表头）在写入前失败；没有改动远端清单，也没有发邮件。
