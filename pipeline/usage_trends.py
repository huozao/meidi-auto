...
def _rows_html(rows: list[dict], months: tuple[str, ...], current_month: str, current_date: str) -> str:
    ...
    headers = ["编号", *(month for month in months), current_month, "今日变化", "单位"]
    out = ["<div class='table-wrap'><table class='trend'><colgroup>",
           "<col class='code-col'>", *["<col class='month-col'>" for _ in (*months, current_month)],
           "<col class='delta-col'><col class='unit-col'></colgroup><thead><tr>"]
    out += [f"<th>{html.escape(label)}</th>" for label in headers] + ["</tr></thead><tbody>"]
    for _, row in pivot.iterrows():
        values = [_short_code(row["code"]), *(row[month] for month in months), row[current_month], _delta(row["今日变化"]), _display_unit(row["unit"])]
        delta_index = len(months) + 2
        cells = []
        for index, value in enumerate(values):
            if 1 <= index < delta_index:
                rendered = _fmt(value)
            elif index == delta_index:
                rendered = value
            else:
...
def _companies_html(rows: list[dict], months: tuple[str, ...], current_month: str, current_date: str) -> str:
    ...
    for rank, company in enumerate(companies, 1):
        ...
        for index, (_, row) in enumerate(pivot.iterrows()):
            ...
            cells.extend(f"<td class='num'>{_fmt(row[month])}</td>" for month in (*months, current_month))
...
def build_usage_sections(data_dir: Path, monthly_root: Path | None = None) -> str:
    ...
    return ("<h2>物料出库趋势</h2>"
            f"<p>统计范围：{months[0]} 至 {current_date}；按近三个月出库总量倒序，今日变化以 +数量表示。</p>"
